...
gen attrition = panel_bal == 0
gen hu = promo_asign == 2
gen np = promo_asign == 3

qui reg attrition hu if promo_asign < 3 & baseline == 1 [pw = ips], cluster(psu)
mat R[5,2] = r(table)[4,1]
qui reg attrition np if (promo_asign == 1 | promo_asign == 3) & baseline == 1 [pw = ips], cluster(psu)
mat R[5,3] = r(table)[4,1]
...
/**********************************************************************/
/*  SECTION 25: Does HU Promotion Increase Health Unit Visits vs. NP?
    Notes: We test whether the $15 voucher (promotion to HU) increased
    self-reported HU visits relative to pure control (NP).
    Outcome: ever_visit_hu_surv — ever visited a health unit (self-reported, survey).
    Specification: FE ITT, restricted to HU vs. NP.                       */
/**********************************************************************/
/* [> load data <] */
use "${dire}analysis_data.dta", clear
...
/* [> restrict to HU vs. NP <] */
keep if promo_asign == 2 | promo_asign == 3
...
/* [> control group mean (NP, endline) <] */
qui mean ever_visit_hu_surv if promo_asign == 3 & endline == 1 [pw = ips], cluster(psu)
local meanc = e(b)[1,1]

/* [> FE ITT: effect of HU promotion on HU visits <] */
qui areg ever_visit_hu_surv promoHU_post endline [pw = ips], abs(woman_id) vce(cluster psu)
qui estadd scalar meanc = `meanc'
est store hu_np_visit

/* [> export results <] */
estout hu_np_visit ///
    using "${dire}hu_np_visithu.xls", ///
    keep(promoHU_post) ///
    cells(b(star fmt(%9.3f)) se(fmt(%9.3f) par)) ///
    stats(meanc N, fmt(%9.3f %9.0fc) labels("Control Group's Mean (NP)" "N")) ///
    collabels(none) varwidth(40) modelwidth(15) unstack label starlevels(* 0.1 ** 0.05 *** 0.01) ///
    mlabels("HU vs. NP") ///
    note("Notes: Dep. variable: The outcome variable takes value 1 if ever visited a health unit (self-reported from survey)." ///
         "FE ITT restricted to HU-promoted and NP groups." ///
         "Standard errors clustered at PSU level in parentheses." ///
         "All observations weighted by the inverse probability of being selected in the sample (IPW)." ///
         "*** p<0.01, ** p<0.05, * p<0.1") ///
    replace
/*------------------------------------ End of SECTION 25 ------------------------------------*/
...
/**********************************************************************/
/*  SECTION 26: ITT Effects of HU Promotion vs. NP on Service Use and Life Satisfaction
    Outcomes: z-scores normalized to NP group (ind_*_znp).
    Specification: FE ITT, restricted to HU vs. NP.                       */
/**********************************************************************/
/* [> load data <] */
use "${dire}analysis_data.dta", clear
...
/* [> list of outcomes (z-scored relative to NP group) <] */
gl depvar_znp ind_tot_znp ind_srh_znp ind_pfe_znp ind_viol_znp ind_pv_znp ind_sea_znp satis_life

est clear


/* [> Romano & Wolf multiple testing p-values <] */
qui rwolf ${depvar_znp} [pw = ips], method(areg) abs(woman_id) indepvar(promoHU_post) controls(endline) vce(cluster psu) cluster(psu) seed(12345) reps(1000)
foreach v in $depvar_znp {
    local rwp_hu_np_`v' = e(rw_`v')
}
...
foreach var in $depvar_znp {
    ...
    tempvar miss_count
    egen `miss_count' = rowmiss(`var' promoHU_post)
    qui mean `var' if `miss_count' == 0 & promo_asign == 3 & endline == 1 [pw = ips], cluster(psu)
    local meanc = e(b)[1,1]
    qui areg `var' promoHU_post endline ${inc_viol} [pw = ips], abs(woman_id) vce(cluster psu)
    qui estadd scalar rwp = `rwp_hu_np_`var''
    qui estadd scalar meanc = `meanc'
    est store hu_np_`var'
}


/* [> export results <] */
estout  hu_np_ind_tot_znp ///
        hu_np_ind_srh_znp ///
        hu_np_ind_pfe_znp ///
        hu_np_ind_viol_znp ///
        hu_np_ind_pv_znp ///
        hu_np_ind_sea_znp ///
        hu_np_satis_life ///
        using "${dire}hu_np_services.xls", ///
        keep(promoHU_post) ///
        cells(b(star fmt(%9.3f)) se(fmt(%9.3f) par)) ///
        stats(rwp meanc N, fmt(%9.3f %9.3f %9.0fc) labels("Romano & Wolf p-value" "Control Group's Mean (NP)" "N")) ///
        collabels(none) varwidth(40) modelwidth(15) unstack label starlevels(* 0.1 ** 0.05 *** 0.01) ///
        mlabels("Aggregate index" "Index sex. and repro. health" "Index prom. employment" "Index violence" "Index patrim. violence" "Index econ. auton." "Life satisfaction") ///
        note("Notes: FE ITT restricted to HU-promoted and NP groups. Z-scores normalized to NP group." ///
             "Standard errors clustered at PSU level in parentheses." ///
             "All observations weighted by the inverse probability of being selected in the sample (IPW)." ///
             "*** p<0.01, ** p<0.05, * p<0.1") ///
        replace
